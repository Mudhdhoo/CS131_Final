...
def _cv_calculate_variation(image, phi, mu, lambda1, lambda2, dt, u_n):
    ...
    if type(u_n) == str:
        ...
        print('Regularized')
        new_phi = (phi + (dt*_cv_delta(phi)) * 
                (mu*K + difference_from_average_term)) + 1*(phi-u_n)
        return new_phi / (1 + mu * dt * _cv_delta(phi) * (C1+C2+C3+C4))
...
def chan_vese(image, mu=0.25, lambda1=1.0, lambda2=1.0, tol=1e-3,
              ...
              extended_output=False, u_n = 'init'):
    ...
    phi = _cv_init_level_set(init_level_set, image.shape, dtype=float)
    ...
    while(phivar > tol and i < max_num_iter):
        ...
        phi = _cv_calculate_variation(image, phi, mu, lambda1, lambda2, dt, u_n)
